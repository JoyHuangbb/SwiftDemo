...
class MobileLoginViewController: BaseViewController {
    ...
    @objc func logindButtonClick(_ button:UIButton) {
        // 登录
        HUDManager.showSuccessHUD("登录成功")
    ...
    private lazy var logindButton: UIButton = {
        ...
        logindButton.setTitle("登录", for: UIControlState.normal)
        ...
        logindButton.addTarget(self, action:#selector(logindButtonClick(_:)), for:.touchUpInside)
    ...
    private lazy var pwdTextField: UITextField = {
        ...
        let pwdTextField = UITextField.init(frame: CGRect.init(x: 20, y: 0, width: ScreenWidth *  0.68 - 80, height: 40))
